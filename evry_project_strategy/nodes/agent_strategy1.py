...
def run_demo():
    ...
    while not rospy.is_shutdown():
        ...
        # Strategy
        velocity = 2.0  # Default forward velocity
        angle = 0.0     # Default straight direction

        distance = float(robot.getDistanceToFlag())
        sonar = robot.get_sonar()
        print(f"{robot_name} distance to flag = {distance}, sonar = {sonar}")

        # Stop if close to the flag
        if distance < 2.0:
            velocity = 0.0
            angle = 0.0
        ...
        # Otherwise, continue moving straight toward the flag
        else:
            velocity = 2.0
            angle = 0.0  # Move forward
